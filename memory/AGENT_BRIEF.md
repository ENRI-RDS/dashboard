...
_Ultimo aggiornamento: 2026-07-03 (rev. 41) — rev.41 ha: in `cluster.html`, non era un bug di rendering ma di leggibilità dato — `_clusterImprese()` usa `IMPRESE_PER_LOTTO` (fallback statico) quando un lotto non ha ancora cantiere reale; quella tabella assegna **3 lotti diversi** ('1','3','7') a "Valtellina", quindi se un cluster contiene più di uno di quei lotti (un lotto attraversa più cluster) l'impresa mostrata si fonde sotto lo stesso nome e cambia solo la somma metri — dava l'impressione di un bug. Fix: aggiunto flag `reale`/`pianificata` per impresa in `_clusterImprese()` (reale = almeno un cantiere realmente avviato su quel lotto, pianificata = nessun cantiere, impresa dedotta solo da `IMPRESE_PER_LOTTO`), badge colorato su ogni card impresa (verde/ambra) + legenda nella sezione. Sistemato anche lo stat card "Lotti / Imprese: 1/1" (poco leggibile, screenshot utente) — separato in due box distinti "Lotti compresi" e "Imprese coinvolte", griglia `det-stats` passata da 4 colonne fisse a `repeat(auto-fit,minmax(120px,1fr))`.

_Ultimo aggiornamento: 2026-07-03 (rev. 40) — rev.40 ha: creata pagina parallela **`cluster.html`** (nuovo file, alternativa a `scavi.html` da valutare internamente) — vista cluster-centrica: sidebar con elenco cluster (% avanzamento, n. lotti, n. imprese), pannello dettaglio con focus primario su "lotti compresi" (chip con % scavo) e "imprese coinvolte" (card per impresa con metri gestiti/scavati e lotti assegnati, fallback su `IMPRESE_PER_LOTTO` per lotti senza cantiere avviato), tabella cantieri del cluster in coda (con bottone Registro, stesso modal pattern di `scavi.html`, z-index:250 già corretto da subito). Riusa stessi endpoint/logica dati di `scavi.html` (`_loadPermessi()` da `Riepilogo_progettazione.csv`, `_loadCantieri()` da `/api/cantieri`, stesso `IMPRESE_PER_LOTTO`/`STATO_MAP`/`STATO_COLOR`), stesso auth guard (`_enri_user`) e brand kit (Raleway, blu monocromatico, niente emoji). **Non portato** da `scavi.html`: script di tracking accessi (log utenti/durata sessione) — da aggiungere se la pagina viene promossa a produzione. Nessuna modifica a `scavi.html` in questa sessione.

_Ultimo aggiornamento: 2026-07-03 (rev. 39) — rev.39 ha: fix bug stacking modal in `scavi.html` — dal modal Stato (card "Stato Cantieri") il click su "Registro" apriva `modalRegistroOverlay` visivamente sotto `modalStatoOverlay` ancora aperto (invisibile/inaccessibile). Causa: entrambi `.modal-overlay` condividevano `z-index:200` (CSS globale riga ~20), a parità di z-index vince l'ordine nel DOM e `modalStatoOverlay` è dichiarato dopo `modalRegistroOverlay` → restava sempre sopra. Fix mirato: `style="z-index:250"` inline su `#modalRegistroOverlay` (riga ~2021), nessun'altra modifica. Da verificare in futuro se altri modal aperti "in cascata" (es. da Lotto o Cluster) soffrono dello stesso problema.
