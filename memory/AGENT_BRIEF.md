...
| — | milestone.html / admin.html | Pagine senza guardia auth completa | Aperto | Alta (sicurezza) | `milestone.html`: nessuna guardia `_enri_user`/overlay, pagina pubblica a chi ha l'URL. `admin.html`: le GET (inventario, coda, assegnazioni) partono senza credenziali, solo le scritture richiedono `UPLOAD_TOKEN`. Segnalato rev.26, mai chiuso |
