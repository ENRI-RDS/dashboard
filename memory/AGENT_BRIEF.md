...
_Ultimo aggiornamento: 2026-07-04 (rev. 112) — rev.112: `mappa_impresa_caricamento.html` — riscritta la guida (`#helpOverlay`, tasto `?`): prima descriveva modalità di visualizzazione/filtri (irrilevanti per l'uso reale dell'impresa) e conteneva 2 voci obsolete mai verificate: export "Immagine" PNG (funzione inesistente nel file, nessun riferimento a canvas/html2canvas/leaflet-image) e nota "aggiorna ricaricando QGIS.geojson" (non pertinente: qui i dati arrivano da API, non da upload manuale file). Nuova guida focalizzata solo sulle funzioni di caricamento reali verificate nel file: Aggiorna pratica, Nuova pratica, Mie richieste, Aggiorna Scavi, ricerca, export GeoJSON tracciato.
_Ultimo aggiornamento: 2026-07-04 (rev. 111) — rev.111: `mappa_impresa_caricamento.html` — **bug reale**: `CAMPO_AWS` non veniva mai rimosso dall'export GeoJSON (né dalle feature tratte/lotto né da quelle SED) nonostante presente in `EXPORT_EXCLUDE_PROPS` (rev.79). Causa: il nome della chiave reale nelle properties è `CAMPO AWS` (con spazio), non `CAMPO_AWS` (underscore) — il `delete props['CAMPO_AWS']` non trovava mai la chiave. Corretto in `EXPORT_EXCLUDE_PROPS`._
