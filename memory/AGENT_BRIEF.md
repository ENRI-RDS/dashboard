...
| — | Master/server | Colonna `DATA_UPDATE` manca in `Master.csv` | Bloccato | Media | Logica frontend (`index.html`, rev.61) e backend (`server.py`, rev.62) già pronte ma no-op: colonna assente nel CSV, la cella mostra sempre "—". Aggiungerla come fatto per `DATA_PREVISTA_RILASCIO` (rev.44) |
