...
23. **Regole stato_cantiere / tecnica_scavo / metri (NEW, rev.5)** — allineate tra `mappa_impresa_caricamento.html` e `imprese_scavi.html`:
    - **Transizioni stato**: `STATO_TRANSITIONS` (JS) limita la select `stato_cantiere` allo stato corrente + transizioni valide (`non_avviato→allestimento→in_corso→{sospeso,completato}`, `sospeso→in_corso`, `completato` terminale). Popolata da `scPopulateStatoOptions()` / `populateStatoOptions()` a ogni apertura form — niente più select statica con tutti gli stati.
    - **`allestimento`**: `tecnica_scavo` e `metri_realizzati_oggi` disabilitati e svuotati in UI, validazione submit e payload inviato — un cantiere in allestimento non richiede né tecnica né metri.
    - **`data_inizio_prevista`**: bloccata solo da `in_corso` in poi (NON da `allestimento`).
    - **`data_fine_prevista`**: editabile solo per `in_corso`/`sospeso`.
    - **`data_inizio_effettiva`**: editabile solo per `in_corso` (`STATI_IN_CORSO_EFF`), e una volta valorizzata diventa permanentemente bloccata (non ri-modificabile).
    - `motivo_blocco`/`data_ripresa_stimata` visibili solo per `sospeso`.
    - ⚠️ **Bug corretto (rev.5)**: `POST /api/imprese/cantieri/{cantiere_key}` confrontava `doc.get("lotto")` (normalizzato, es. `"1A"`) con `assignment.lotti` **non normalizzato** (es. `"Lotto 1A"`) → 403 "Pratica non assegnata a questa impresa" anche per lotti realmente assegnati. Fix: entrambi i lati passano ora da `_lotto_from_source()`, come già faceva `GET /api/imprese/cantieri`.
24. **Storico cantiere (rev.6)**: `log_entry` in `update_cantiere` (`server.py`) ora include anche `tecnica_scavo` (mancava — impossibile mostrarla in storico). Tabella storico in `mappa_impresa_caricamento.html`/`imprese_scavi.html` (già presente in entrambe) allineata: colonna "Tecnica" aggiunta (`SC_TECNICA_LABEL`/`TECNICA_LABEL`), header "Metri oggi" rinominato in "Metri". Voci di log salvate prima del deploy non hanno `tecnica_scavo` (mostrano "—").
...
_Ultimo aggiornamento: 2026-07-01 (rev. 13) — rev.13 ha: `data_inizio_prevista` obbligatoria per passare a stato `allestimento` — validato client (`imprese_scavi.html`, `mappa_impresa_caricamento.html`) e server (`update_cantiere`). rev.12 ha: **bug fix** `data_inizio_effettiva`/`data_fine_effettiva` non erano obbligatorie — validazione aggiunta sia client-side (`imprese_scavi.html`, `mappa_impresa_caricamento.html`) sia server-side (`update_cantiere` in server.py): blocco se si passa a `in_corso` senza `data_inizio_effettiva` (solo al primo avvio, non già registrata) o a `completato` senza `data_fine_effettiva`. rev.11 ha: **bug fix** `/api/files` mostrava "Ultima modifica" statica (mtime del deploy) per i file gestiti solo via push GitHub e non tracciati in GridFS (`cantieri.csv`, `sopralluoghi.csv`, `solleciti.csv`, ecc.) — i dati venivano davvero aggiornati e pushati su GitHub, ma la UI admin non lo mostrava, sembrando "non aggiornato". Aggiunto `_GITHUB_PUSH_TIMES` (dict in-memory aggiornato ad ogni push riuscito in `_push_to_github`), usato ora da `/api/files` al posto del mtime su disco quando disponibile (si resetta ad ogni restart del processo, ma riflette l'attività reale da allora). rev.10 ha: stesso pattern reset esteso a `sopralluoghi` — nuovo `DELETE /api/admin/sopralluoghi/reset` (server.py) + bottone "Reset" sulla riga `sopralluoghi.csv` in admin.html "File correnti". rev.9 ha: aggiunto bottone "Reset" sulla riga `cantieri.csv` in `admin.html` tab "File correnti" → chiama `DELETE /api/admin/cantieri/reset` (rev.8) con confirm dialog, invece di dover usare curl a mano. rev.8 ha: **nuovo endpoint** `DELETE /api/admin/cantieri/reset` (richiede `x-upload-token`) — svuota `cantieri` e rilancia `_sync_cantieri()`, per reset rapido durante i test. **Bug fix**: `metri_realizzati_oggi` non aveva alcun cap — era possibile superare `metri_totali` e far salire la % oltre 100%; ora `POST /api/imprese/cantieri/{key}` rifiuta (400) se `metri_oggi > rimanenti` (server.py); aggiunto hint "Rimangono Xm su Ym" + blocco client-side identico in `imprese_scavi.html` e `mappa_impresa_caricamento.html`. rev.7 ha: **bug fix** `TECNICA_SCAVO_VALUES` in `server.py` mancava `"canaletta"` (presente come opzione in entrambi i frontend) → `POST /api/imprese/cantieri/{key}` rispondeva 400 e bloccava l'intero salvataggio (anche stato/metri) quando l'impresa selezionava "Canaletta"; aggiunto `tecnica_scavo` alla card `scRenderList()` in `mappa_impresa_caricamento.html` (presente in `imprese_scavi.html` ma assente nella lista cantieri della mappa). rev.6 ha: aggiunto `tecnica_scavo` a `log_entry` in `update_cantiere` (mancava), colonna "Tecnica" e rename "Metri oggi"→"Metri" nello storico di `mappa_impresa_caricamento.html`/`imprese_scavi.html` (§8.24). rev.5 ha: allineate le regole stato_cantiere/tecnica_scavo/metri tra `mappa_impresa_caricamento.html` e `imprese_scavi.html` — transizioni stato via `STATO_TRANSITIONS`, blocco tecnica/metri su `allestimento`, lock permanente `data_inizio_effettiva`, visibilità date corrette (§8.23); corretto bug 403 in `POST /api/imprese/cantieri/{cantiere_key}` per confronto lotto non normalizzato (§8.23). rev.4 ha: introdotto `codice_cantiere` (CA/progressivo/lotto) come identificativo stabile e permanente del cantiere, assegnato una sola volta in `_sync_cantieri()` con backfill per i cantieri storici (§9, §5.4, §8.22); aggiornati `mappa_impresa_caricamento.html` e `imprese_scavi.html` per mostrarlo come riferimento primario al posto del calcolo client-side per indice (bug di stabilità corretto). rev.3 ha: completato il rebranding Retelit su tutte le pagine (§8.11, topbar navy, logo base64 bianco 50px, title tag, token CSS completi su tutti i file); rimosso definitivamente `executive_summary.html` e ogni riferimento residuo (§3, §8.17); rinominato "Lotti in Avvio" → "Lotti in Progettazione" e rimossa colonna milestone ridondante da `index.html` (§8.18); esteso `IMPRESE_PER_LOTTO` a tutti i 12 lotti (§8.19); unificato i bottoni Excel SED in un dropdown (§8.20); documentato `debug_dashboard.py` (§8.21)._
