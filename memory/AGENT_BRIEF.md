...
| — | server.py | Endpoint non-impresa senza controllo ruolo (solo token valido) | Chiuso (rev.129) | Alta (sicurezza) | `_require_staff_session` (403 su ruolo `impresa`) su `lotti-cantieri`, `sopralluoghi`×3, `cantieri`, `cantieri/{key}/log` |
...
_Ultimo aggiornamento: 2026-07-04 (rev. 113) — rev.113: `imprese.html` — tab "Aggiorna pratiche": selezionando stato `NO COMPETENZA` ora nasconde Nulla Osta Necessario, Tratte NO, Polizza necessaria, Convenzione (campi non richiesti — richiesta utente: "non deve essere necessario inserire altro se non note eventuali"). Reset dei relativi valori alla selezione. Campi esclusi dal payload inviato (`addToQueueBtn`) anche se valorizzati in precedenza. Data prevista rilascio già nascosta da rev.44 per questo stato. Non toccato tab "Nuova pratica" (non prevede stato NO COMPETENZA in creazione).
_Ultimo aggiornamento: 2026-07-04 (rev. 112) — rev.112: `imprese.html` — rimosso CSS morto `.topbar-back.dim`/`.topbar-back.dim:hover` (residuo di rev.111, classe `dim` non più applicata a nessun elemento dopo l'uniformazione del tasto Hub). TODO §11 "Invio Preliminare" chiuso: verificato già implementato in `STATO_TRANSITIONS`/`_populateStatoSelect`.
_Ultimo aggiornamento: 2026-07-04 (rev. 111) — rev.111: `imprese.html` — topbar: (1) tasto "Hub" aveva classe `topbar-back dim` (colore/bordo attenuati), disomogeneo rispetto a Scavi/Mappa/Aggiorna Mappa — rimossa `dim`, ora uniforme; (2) `.topbar-back` non resettava gli stili UA di `<button>` (stesso gap già risolto in `mappa_impresa_caricamento.html` rev.93) — aggiunto `background:none;font-family:inherit;cursor:pointer;appearance:none;-webkit-appearance:none`; (3) nuovo tasto "?" (stessa classe `.topbar-back`, elemento `<button>`) che apre modal guida (`_openHelp()`, riusa stile `.sub-modal` esistente) con 5 passi essenziali per l'impresa: cerca pratica → modifica campi → controlla riepilogo modifiche → invia per approvazione → naviga con gli altri tasti topbar. Testo volutamente semplice, nessun dettaglio tecnico (workflow pending_updates, MongoDB, ecc.) essendo rivolto all'impresa.
_Ultimo aggiornamento: 2026-07-04 (rev. 110) — rev.110: `scavi.html` — icona sort M.Tot/M.Scavati ancora invisibile dopo rev.108 (rimosso solo il filtro, ma `max-width:70px` restava troppo stretto per label+icona). Max-width dedicato 100px per queste due colonne.
_Ultimo aggiornamento: 2026-07-04 (rev. 109) — rev.109: `scavi.html` — colonna Ente 180px troncava "COMUNE DI LACCHIARELLA" ecc. (screenshot utente). Portata a 240px.
_Ultimo aggiornamento: 2026-07-04 (rev. 108) — rev.108: `scavi.html` — tabella "Tutti i Cantieri": (1) colonna Ente troppo larga (prendeva tutto lo spazio libero, screenshot: icone sort/filtro di Ente finivano visivamente addossate a Lotto) — ora `width:180px` fissa + ellipsis/tooltip su header e celle; (2) M. Tot./M. Scavati: rimossa icona filtro (`noFilter:true`, filtro su valori numerici poco utile) per liberare spazio all'icona sort, non visibile prima nella colonna stretta.
_Ultimo aggiornamento: 2026-07-04 (rev. 107) — rev.107: `scavi.html` — rimosso CSS morto (0 usi HTML/JS, verificato con analisi grep mirata): blocco `.msc-*` (10 regole, vecchio stile card modal "Stato Cantieri" pre-rev.77, sostituito da tabella), `.page-eyebrow`/`.page-h1`/`.page-sub` (mai usate in questo file, probabile residuo copia da `index.html`), `.demo-badge`, `.alert-banner`, `.cluster-section`/`.cluster-sep`, `.bar-lotto-comuni`/`.bar-middle`/`.bar-info`/`.bar-meters` (markup ora generato inline via `style.cssText` in `_renderBars`/`_renderClusters`), `.kpi-filt-btn` (rimpiazzata da `.kpi-filt-select`), blocco `.placeholder-*` (nessun placeholder attivo). `.bar-pct`/`.bar-track`/`.bar-seg` mantenute (in uso).
_Ultimo aggiornamento: 2026-07-04 (rev. 106) — rev.106: `scavi.html` — `lottiSubtitle`: rimossa dicitura "· permessi (sopra) · % metri scavati (sotto)", resta solo "X m totali".
_Ultimo aggiornamento: 2026-07-04 (rev. 105) — rev.105: `scavi.html` — `.btn-registro` rimpicciolito (padding 5px 11px→4px 9px, font 12px→11px, icona 12px→11px, gap 5px→4px) per liberare spazio alla colonna "M. Scavati" accanto, troppo stretta (screenshot utente: header troncato "M. AVATI"). Nessun `table-layout:fixed` in `.cant-table`, quindi lo spazio liberato viene ridistribuito automaticamente dal browser.
_Ultimo aggiornamento: 2026-07-04 (rev. 104) — rev.104: `scavi.html` — `.kpi-pct` (badge % nelle card "Stato Cantieri": 91%/7%/3%/0%/0%) font-size 12px→16px, troppo piccolo rispetto alle % equivalenti di `index.html` (screenshot utente, confronto diretto).
_Ultimo aggiornamento: 2026-07-04 (rev. 103) — rev.103: `scavi.html` — tabella "Tutti i Cantieri", header: (1) font-size 12px→11px (richiesta utente, screenshot: disallineamento percepito); (2) `.th-cell` da `align-items:flex-start` a `center` — le icone sort/filtro erano disallineate verticalmente rispetto alla label quando l'header andava a capo; (3) colore icone `var(--muted)`→`var(--text)` e dimensione 18px/11px→19px/12px, poco visibili su sfondo `--surface2` (contrasto basso, segnalato in più occasioni).
_Ultimo aggiornamento: 2026-07-04 (rev. 102) — rev.102: `scavi.html` — "Avanzamento per Lotto": (1) caption card ora mostra `<n> m totali` (somma `LOTTI[].metri`) come in "Avanzamento per Cluster" (`clusterStatus`), prima assente; (2) label lotto: aggiunto nome impresa assegnataria (`IMPRESE_PER_LOTTO[r.id]`) tra id lotto e badge province; (3) province ordinate alfabeticamente alla fonte (`LOTTI[].prov`, in `_loadCantieri()`) invece dell'ordine CSV/Set non deterministico — coerente ovunque `r.prov` è letto (barre, modal lotto, tabella cantieri del lotto).
_Ultimo aggiornamento: 2026-07-04 (rev. 101) — rev.101: `scavi.html` — tabella "Tutti i Cantieri": rimossa l'icona filtro-per-colonna (funnel SVG) sulle colonne Lotto/Stato/Cluster/Impresa, poco leggibile affiancata all'icona sort e ridondante coi 4 select "quick filter" già presenti sopra la tabella (Tutti i lotti/Tutti gli stati/Tutti i cluster/Tutte le imprese). Resta solo l'icona sort su queste colonne; le altre (Cod. Cantiere, Pratica, Ente, Prov., Comune, Avanz., M. Tot., M. Scavati) mantengono entrambe le icone, non essendo coperte da alcun quick filter. Implementato in `_renderCantieriTableHead()` con check `CANTIERI_QUICK_FILTERS.some(f => f.key === c.key)`. Verificati anche in questa sessione, su richiesta di controllo TODO: (a) font-size <12px in `scavi.html` (§11) — grep mirato, nessun residuo, TODO chiuso; (b) bug `_setCantieriLottoFiltro` non definita (rev.94) — funzione non più referenziata nel file, risolta insieme alla sostituzione bottoni→select di rev.94.
_Ultimo aggiornamento: 2026-07-03 (rev. 100) — rev.100: `index.html` — tabella "Tutte le pratiche": icone ordinamento (freccia unicode `↕`/`↑`/`↓`, 9px opacity .35, poco visibile — screenshot utente) sostituite con icona SVG a doppio triangolo (stessa di `scavi.html`/"Tutti i Cantieri"), colore accent, triangolo attivo a opacità piena. Nuovo helper `_sortIconHtml(dir)` condiviso da `buildThead()` e dal refresh post-sort.
_Ultimo aggiornamento: 2026-07-03 (rev. 99) — rev.99: `index.html` — colonna `Ente` (`w:'auto'`) assorbiva tutto lo spazio liberato dalla rimozione di Scadenza (rev.95) con `table-layout:fixed`, causando spazi vuoti enormi in header (screenshot utente). Fissata a `170px` + `white-space:nowrap;overflow:hidden;text-overflow:ellipsis` su `.tbl-ente` (title già presente per tooltip); spazio residuo ora ridistribuito proporzionalmente su tutte le colonne dal layout fixed invece che scaricato su una sola.
_Ultimo aggiornamento: 2026-07-03 (rev. 98) — rev.98: `index.html` — rimossi bottoni filtro scadenza "Tutte/⚠ Scadute/Scad. 7gg/Scad. 14gg" sopra "Tutte le pratiche" e tutto il codice diventato morto: `tblSetScad`, variabile `_scadW`, `window._tblScadW`, `wLabel` (calcolata ma mai letta), filtro scadenza in `exportVistaCorrente`, campi `prev`/`scad_gg` in `getAllPrats()` (non più letti da nessun consumer). `calcolaPrevisione` resta usata dal widget "Prossime scadenze" in sidebar, non toccata. `tblReset` semplificata (non chiama più `tblSetScad(0)`).
_Ultimo aggiornamento: 2026-07-03 (rev. 97) — rev.97: `index.html` — rimossa regola CSS morta `.topbar-right a[href="executive_summary.html"]::after` (pagina non più esistente, segnalato dall'utente).
_Ultimo aggiornamento: 2026-07-03 (rev. 96) — rev.96: `index.html` — aggiunto tasto topbar "Avanz. Lavori" → `scavi.html`, accanto a "Mappa".
_Ultimo aggiornamento: 2026-07-03 (rev. 95) — rev.95: `index.html` — rimossa colonna "Scadenza" dalla tabella "Tutte le pratiche" (`COLS`, cella riga, case sort `_sortK==='scad'`) e funzione `scadHtml()` (diventata inutilizzata). `scad_gg`/`calcolaPrevisione` lasciati intatti in questa fase (ancora referenziati altrove, v. rev.98 per pulizia successiva).
_Ultimo aggiornamento: 2026-07-03 (rev. 94) — rev.94: `scavi.html` — "Tutti i Cantieri": rimossi i bottoni per singolo lotto, sostituiti con 4 select filtro (Lotto/Stato/Cluster/Impresa), coordinati col filtro Excel-style già esistente per colonna. **Bug trovato**: `_setCantieriLottoFiltro` era referenziata nell'`onclick` dei vecchi bottoni ma mai definita — i bottoni non hanno mai filtrato nulla.
_Ultimo aggiornamento: 2026-07-03 (rev. 93) — rev.93: `scavi.html` — **bug reale**: card "Avanzamento per Lotto" bloccata su "caricamento…" (screenshot utente). Causa: `_renderPageSubtitle()` faceva `return` anticipato se non trovava l'elemento `pageSubDyn` (residuo copiato da `index.html`, mai esistito in `scavi.html`), saltando l'aggiornamento di `lottiSubtitle` che stava dopo nella stessa funzione. Fix: `lottiSubtitle` ora aggiornato indipendentemente dalla presenza di `pageSubDyn`. Inoltre: subtitle "Performance Imprese" semplificato ("3 imprese · ordinate per Avanzamento ↓ · clicca una colonna per cambiare ordine" → "3 imprese · ord. Avanzamento ↓", hint spostato in `title` tooltip), CSS `.cc-sep` con margine mancante aggiunto globalmente.


_Ultimo aggiornamento: 2026-07-04 (rev. 93) — rev.93: `mappa_impresa_caricamento.html` — fix uniformità visiva topbar segnalato dall'utente (screenshot: bottoni "⬇ GeoJSON" e "?" più marcati/in grassetto rispetto a Pratiche/Mappa/Scavi/Hub pur avendo la stessa classe `.topbar-back`). Causa: quei due sono elementi `<button>` mentre gli altri sono `<a>` — `.topbar-back` non resettava gli stili UA di default del bottone (`background`, `font-family`, `appearance`), che su Chrome/Edge applicano un font più pesante e uno sfondo leggermente diverso. Aggiunta a `.topbar-back`: `background:none;font-family:inherit;cursor:pointer;appearance:none;-webkit-appearance:none;` — ora tutti i pulsanti topbar (link e bottoni) rendono identici.
_Ultimo aggiornamento: 2026-07-04 (rev. 92) — rev.92: `mappa_impresa_caricamento.html` — bug reale in `exportGeoJSONLotti()` segnalato dall'utente ("dati delle tratte uniti con i SED, cluster non estratto"). Causa: (1) tratte (`allFeatures`, LineString) e SED (Point) venivano concatenate in un'unica `FeatureCollection` senza alcun campo che le distinguesse — schemi di proprietà completamente diversi fusi in un blocco indistinguibile una volta aperto in QGIS/GIS; (2) il campo cluster viene letto altrove nel file sempre con fallback `p.CLUSTER ?? p.ID_CLUSTER ?? p.cluster` (righe 1197/1362/1421/1428/1480), segno che nel dataset sorgente il nome campo non è coerente — l'export invece copiava le `properties` raw senza normalizzazione, quindi se una tratta aveva il valore sotto `ID_CLUSTER` l'export non produceva mai una chiave `CLUSTER`. Fix: aggiunto `properties.layer` = `'TRATTA'`/`'SED'` a ogni feature esportata; aggiunta normalizzazione esplicita `props.CLUSTER = f.properties.CLUSTER ?? f.properties.ID_CLUSTER ?? f.properties.cluster ?? null` (solo per le feature `TRATTA`, le SED non hanno mai cluster) prima della rimozione dei campi in `EXPORT_EXCLUDE_PROPS`.
...
_Ultimo aggiornamento: 2026-07-04 (rev. 80) — rev.80: `mappa_impresa_caricamento.html` — rimossi funzione `exportPDF()`, tasto "⬇ PDF" in toolbar, regola CSS mobile che lo nascondeva, riferimento nel testo guida (`openHelp`). Rimasto orfano (non rimosso, non causa bug) il blocco `@media print{...}` a riga ~273, usato solo da `exportPDF()`/`window.print()` — dimmi se vuoi che lo tolga anche quello.
_Ultimo aggiornamento: 2026-07-04 (rev. 79) — rev.79: `mappa_impresa_caricamento.html` — `exportGeoJSONLotti()` esclude ora questi campi dalle `properties` del GeoJSON esportato (dati interni/gestionali non da consegnare all'esterno): `MOTIVO_NO`, `STATO_AUTORIZZAZIONE`, `STATO_NULLAOSTA`, `STATO_ORDINANZA`, `PROTOCOLLO_AUT`, `PRATICA`, `LAVORABILE`, `STATO_LEGENDA`, `CAMPO_AWS`. Lista in `EXPORT_EXCLUDE_PROPS`, applicata sia alle feature del lotto sia a quelle SED se incluse. Non tocca i dati usati a runtime nei popup/UI (solo lo snapshot esportato).
_Ultimo aggiornamento: 2026-07-04 (rev. 78) — rev.78: `mappa_impresa_caricamento.html` — `exportGeoJSONLotti()` usava `confirm()`/`alert()` nativi del browser per chiedere se includere gli attraversamenti SED nell'export (era già funzionante, screenshot utente lo mostrava come popup "enri-rds.github.io dice"). Sostituiti con `prConfirm()`/`prAlert()`, il modal custom brand-kit già presente nel file (`#prConfirmOverlay`, usato altrove es. cancellazione pratiche), per coerenza visiva. Bottoni rinominati "Sì, includi SED" / "No, solo lotto" al posto di "OK"/"Annulla" generici.
...
_Versione compattata il 2026-07-03: changelog storico (rev.10-64) spostato in `AGENT_BRIEF_ARCHIVE.md`, tabella TODO §11 aggiornata con 2 voci emerse dall'analisi (DATA_UPDATE, gap auth milestone/admin), corretta riga COORDINAMENTO→INVIO PRELIMINARE in §8bis (era rimasta disallineata dal rename rev.34-43)._
