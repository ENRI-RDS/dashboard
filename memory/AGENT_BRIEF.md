...
| 16 | Imprese, Mappa_imprese_caricamento | Per lo stato NO COMPETENZA non necessaria richiesta di previsione rilascio | Chiuso (rev.52) | — | Campo nascosto/non inviato anche per NO COMPETENZA (prima solo OTTENUTO), in entrambi i file. Form "nuova pratica" non necessita fix analogo: stato sempre hardcoded `IN REDAZIONE` alla creazione |
| — | Milestone | Nessuna guardia auth — pagina completamente pubblica a chi conosce l'URL | Da fare | Media | Manca il pattern `_enri_user`+overlay già presente su scavi.html/sopralluoghi.html |
| — | Admin | GET (file correnti, storico versioni, coda imprese, assegnazioni) partono senza credenziali — solo le scritture sono protette da `UPLOAD_TOKEN` | Da fare | Media | Bloccare le GET automatiche finché `enri_upload_token` non è valorizzato |
| — | Backend | Master.csv malformato in upload: `_regenerate_derived_files()` fallisce in silenzio (log solo su stdout), QGIS/Riepilogo restano non rigenerati senza segnale visibile | Da fare | Bassa | Esporre `last_regen_error` in un endpoint di stato, o validare colonne chiave attese e rigettare l'upload con errore esplicito |
| — | Backend | `/api/data`, `/api/data-text`, `/api/preview` (server.py) senza check `x-session-token` — CSV scaricabili da chiunque conosca l'URL Render, nessuna auth reale | Da verificare | Media | `_require_session` già esiste (usata su `/api/imprese/*`), riusabile qui. Nota: `index.html` legge i CSV da GitHub Pages statico, non da questi endpoint — proteggerli non copre quella pagina. Da applicare su: pagine che chiamano `/api/data*` con `x-session-token` già disponibile (hub.html, imprese_scavi.html, scavi.html, mappa*.html, admin.html — da confermare quali usano davvero questi 3 endpoint, non ancora verificato file per file) |

---

**Note operative permanenti** (non modifiche puntuali — tenere sempre presenti, non serve rileggerle dal changelog):
- **Master.csv è tab-separated**, non comma/semicolon. Prima di editare un file ricevuto, verificare con `head -1 file | cat -A`: se ogni riga appare come un unico blob tra virgolette, il file è corrotto (probabile export/copia-incolla Excel malformato) e va riconvertito con `csv.reader(delimiter=',')` prima di qualunque modifica.
- `.modal-bar-pct` (percentuale nelle barre "Dettaglio" dei modal lotto/cluster, `index.html`) ha avuto ~10 iterazioni di stile/posizionamento prima di stabilizzarsi: percentuale sempre centrata al 50% del track, testo scuro (`var(--gray-700)`) con outline chiaro via `text-shadow`, nessun pill/background/blend-mode. Non serve riaprire la questione salvo richiesta esplicita.
- `_apply_changes_to_df()` in `server.py` non ha whitelist campi (salvo `_POL_CONV_ALLOWED_FIELDS` per CONVENZIONE/POLIZZA): qualunque campo frontend che scriva su una colonna già esistente in Master.csv funziona senza toccare il backend.

_Ultimo aggiornamento: 2026-07-03 (rev. 52) — rev.52 ha: chiuso checklist #16 — campo "Data prevista rilascio" nascosto/non inviato anche per stato `NO COMPETENZA` (prima solo per `OTTENUTO`), in `imprese.html` (`wrap-prev-rilascio` toggle + `prevRilascio` in submit) e `mappa_impresa_caricamento.html` (`pr-wrap-data-prevista` toggle + `dataPrevista` in submit); hint testuali aggiornati in entrambi. Overlay login `index.html` reso opaco+blur (`rgba(4,40,75,.75)` + `backdrop-filter:blur(18px)`); `loadSED()`/`window.load` (loadMaster/loadData/loadCluster/loadSolleciti) ora non partono se `localStorage._enri_user` è assente._
