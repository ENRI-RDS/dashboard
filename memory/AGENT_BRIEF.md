...
_Ultimo aggiornamento: 2026-07-08 (rev. 155) — rev.155: `gantt.html` + `backend/server.py`, richiesta utente: dipendenze tra task ora **editabili**, non più solo in sola lettura. In "Modalità modifica" (admin/admin2) il popover di editing di ogni riga mostra ora `ganttDepsEditHtml()` (select Predecessore + select Tipo FS/SS + input Ritardo giorni) al posto della vecchia vista testuale `ganttDepsHtml()` (quest'ultima resta usata solo nel popover read-only fuori da edit mode, invariato). Salvataggio: `saveGanttEdit()` include `dep_pred`/`dep_type`/`dep_lag` nel payload esistente (stesso bottone Salva, stesso endpoint `PUT /api/gantt/overrides/{lotto}/{row_id}`), validazione client self-reference (pred≠rowId). `applyGanttOverrides()` applica l'override alla mappa runtime `GANTT_DEPS` (aggiunge/sostituisce se `dep_pred` valorizzato, cancella la entry se `dep_pred` vuoto/null = "rimuovi dipendenza"). Backend: whitelist campi PUT estesa con `dep_pred`/`dep_type`/`dep_lag` (validati: pred deve essere intero ≠ row_id, type in FS/SS, lag intero ≥0), GET restituisce anche questi 3 campi. Nessuna modifica a `GANTT_DEPS` baseline (dati .mpp rev.154) né a `gantt_overrides` collection/schema Mongo (stessi documenti, solo nuove chiavi facoltative). Non implementato: validazione cicli multi-hop (solo self-reference bloccata) — dipendenze successori restano editabili solo dal popover del successore stesso, non dal predecessore. `py_compile`/`node --check` OK.
