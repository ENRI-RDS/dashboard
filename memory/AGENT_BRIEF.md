...
_Ultimo aggiornamento: 2026-07-03 (rev. 78) — rev.78: `scavi.html` — follow-up immediato a rev.77(e): il modal "Stato Cantieri" era rimasto a `width:560px` inline (dimensionato per la vecchia lista card), causando scroll orizzontale forzato ora che contiene la tabella a 13 colonne di "Tutti i Cantieri" (bug segnalato dall'utente via screenshot: solo le prime 7 colonne visibili senza scorrere). Portato a `width:1180px;max-width:96vw` (v. §4.39.f).
