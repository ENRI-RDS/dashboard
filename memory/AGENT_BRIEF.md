...
| **polizze_convenzioni.html** | ✅ **RISOLTO** — guardia di login (`_enri_user`) aggiunta, stesso pattern overlay usato in scavi.html/sopralluoghi.html (vedi file corretto). La sola scrittura (`POST /api/admin/polizze-convenzioni/update`) richiede un `UPLOAD_TOKEN` chiesto al volo via modale e salvato in `sessionStorage['_enri_upload_token']` (chiave **diversa** da quella usata in admin.html, vedi §8.15 — non ancora risolto) | Pratiche con CONVENZIONE e/o POLIZZA richiesta: filtro per lotto/impresa/stato, KPI aggregati. Legge `/api/admin/polizze-convenzioni/data-richiesta`, con fallback diretto a Master.csv su GitHub Pages se l'API non risponde |
