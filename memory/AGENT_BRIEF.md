...
| — | All | Rename `COORDINAMENTO`→`INVIO PRELIMINARE` | Risolto (rev.40) | — | Verificato file per file con grep mirato dopo lo scarto delle voci fabbricate (rev.39): Master.csv/server.py già ok, scavi.html e mappa_impresa.html corretti in rev.40, mappa.html/mappa_impresa_caricamento.html già ok |
