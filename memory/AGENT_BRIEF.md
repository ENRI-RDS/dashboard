...
_Ultimo aggiornamento: 2026-07-03 (rev. 51) — rev.51 ha: segnalato in §11 (nuova riga TODO) — `/api/data/{filename}`, `/api/data-text/{filename}`, `/api/preview/{filename}` in `server.py` non hanno alcun controllo `x-session-token`/auth, a differenza di `/api/imprese/*` che usano `_require_session`. Nessuna modifica al codice in questa sessione, solo segnalazione da verificare/decidere in sessione futura (quali pagine coprire, se serve davvero dato che `index.html` non passa da questi endpoint). Nella stessa sessione: overlay login `index.html` reso opaco+blur (`rgba(4,40,75,.75)` + `backdrop-filter:blur(18px)`, era `rgba(4,40,75,.6)` senza blur — lasciava intravedere la dashboard sotto il modal); `loadSED()` e il listener `window.load` (loadMaster/loadData/loadCluster/loadSolleciti) ora non partono se `localStorage._enri_user` è assente, invece di fare comunque fetch/render dei dati prima del check auth.
